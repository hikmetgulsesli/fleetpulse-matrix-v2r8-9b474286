...
- FleetPulse Matrix V2R8 turns the user's request into a directly usable web workflow. The first experience must be the actual web product behavior, not a marketing landing page or placeholder demo.
- - FR-001: Build a compact browser fleet maintenance scheduling app called FleetPulse Matrix V2R8. It should manage vehicles, preventive maintenance plans, technician queues, overdue service tasks, settings, empty and error recovery, and every visible action should update real app state.
...
- Vehicle Operations - FleetPulse Matrix V2R8
- Vehicle Editor - FleetPulse Matrix V2R8
- Queue and Status Management - FleetPulse Matrix V2R8
- Settings and Preferences - FleetPulse Matrix V2R8
- Empty and Error Recovery - FleetPulse Matrix V2R8

## SCREEN_SPECS
SCREEN_SPEC_1:
- exact_screen_title: Vehicle Operations - FleetPulse Matrix V2R8
- surface_id: SURF_VEHICLE_OPERATIONS
- unique_canvas_caption: Vehicle Operations: summary metrics, primary list/board/table, filters, search, selected item preview, empty/loading/error states.
- purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Vehicle data.
...
- data_entities: Vehicle, ActivityEvent, Preference
...
SCREEN_SPEC_2:
- exact_screen_title: Vehicle Editor - FleetPulse Matrix V2R8
- surface_id: SURF_VEHICLE_EDITOR
- unique_canvas_caption: Vehicle Editor: form fields, required/optional indicators, validation messages, save/cancel controls, unsaved-state feedback.
- purpose: Let the user create, edit, validate, save, cancel, and recover Vehicle changes.
...
- data_entities: Vehicle, ValidationError
- visible_actions: ACT_SAVE_RECORD as form_submit, ACT_CANCEL_EDIT as secondary_button
- entry_exit_rules: SURF_VEHICLE_OPERATIONS -> Save returns to SURF_VEHICLE_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
- design_guidance: Form layout must be clear and task-specific; do not invent payment, onboarding, or unrelated identity forms.

SCREEN_SPEC_3:
- exact_screen_title: Queue and Status Management - FleetPulse Matrix V2R8
...
- purpose: Help users organize Vehicle work by queue, status, SLA, stage, priority, or triage context when those signals are part of the requested product.
...
- entry_exit_rules: SURF_VEHICLE_OPERATIONS -> Status changes keep the user in the same Vehicle context and expose recoverable failure feedback.
- design_guidance: Represent operational state clearly; do not flatten queue/SLA work into decorative metrics.

SCREEN_SPEC_4:
- exact_screen_title: Settings and Preferences - FleetPulse Matrix V2R8
...
- purpose: Let users adjust Vehicle workflow preferences, saved filters, defaults, or product settings requested by the task.
...
- entry_exit_rules: SURF_VEHICLE_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
- design_guidance: Settings must support the requested workflow only; do not invent unrelated profile or billing areas.

SCREEN_SPEC_5:
- exact_screen_title: Empty and Error Recovery - FleetPulse Matrix V2R8
...
- purpose: Keep the Vehicle workflow usable when data is missing, filtered away, loading, failed, or corrupt.
...
- entry_exit_rules: SURF_VEHICLE_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Vehicle workflow and preserves unrelated state.
...
## PRODUCT_SURFACES
1. SURF_VEHICLE_OPERATIONS - Vehicle Operations
   Purpose: Give the user the main operational view for inspecting, searching, filtering, and acting on Vehicle data.
   Data: Vehicle, ActivityEvent, Preference
...
2. SURF_VEHICLE_EDITOR - Vehicle Editor
   Purpose: Let the user create, edit, validate, save, cancel, and recover Vehicle changes.
   Data: Vehicle, ValidationError
   ...
   Entry/exit: SURF_VEHICLE_OPERATIONS -> Save returns to SURF_VEHICLE_OPERATIONS with persisted changes; cancel preserves existing data and closes the editor.
...
3. SURF_QUEUE_AND_STATUS_MANAGEMENT - Queue and Status Management
   ...
   Entry/exit: SURF_VEHICLE_OPERATIONS -> Status changes keep the user in the same Vehicle context and expose recoverable failure feedback.
...
4. SURF_SETTINGS_AND_PREFERENCES - Settings and Preferences
   ...
   Entry/exit: SURF_VEHICLE_OPERATIONS -> Saved preferences immediately affect visible product state or show a clear confirmation.
...
5. SURF_EMPTY_AND_ERROR_RECOVERY - Empty and Error Recovery
   ...
   Entry/exit: SURF_VEHICLE_OPERATIONS, SURF_INSIGHTS -> Recovery returns to the active Vehicle workflow and preserves unrelated state.
